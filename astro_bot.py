...
async def show_natal_chart_callback(callback_query: types.CallbackQuery):
    ...
    prompt = (
        ...
        f"В свой ответ в конце вставь вопросительную фразу, типа Чем я еще могу попочь\n"
...
async def daily_horoscope_callback(callback_query: types.CallbackQuery):
    ...
    user_first_name = user.get('first_name', 'друг')  # Получаем имя пользователя
 	...
 	prompt = (
		f"Ты — опытный астролог, составляющий персональный гороскоп на день по натальной карте и текущим транзитам.\n"
		f"Вот данные пользователя:\n"
		f"- Дата рождения: {user['birth_date']}\n"
		f"- Время рождения: {user['birth_time']}\n"
		f"- Место рождения: {user['birth_place']}\n\n"
		f"Составь подробный **гороскоп на сегодня**, используя реальные астрологические аспекты и тенденции дня.\n\n"
		f"Структура гороскопа:\n"
		f"1. 🔮 Общая атмосфера дня\n"
		f"2. 💭 Эмоциональное состояние\n"
		f"3. 💼 Работа и финансы\n"
		f"4. ❤️ Любовь и отношения\n"
		f"5. ⚠️ Осторожность и рекомендации\n"
		f"6. 💡 Советы дня\n\n"
		f"Стиль: вдохновляющий, понятный, без сложных терминов. Пиши живо и дружелюбно. "
		f"Выделяй важные моменты **жирным шрифтом** и используй уместные эмодзи.\n"
		f"В конце добавь тёплый вопрос, например: *Чем я ещё могу помочь?* 😊"
)
